...
// Given the scrollbar widths of some other container, create borders/margins on rowEls in orderflow to match the left
...
Grid.mixin({
	...
	// Doesn't guarantee an orderflow for the resulting array.
	...
	invertRanges: function(ranges) {
		...
		// ranges need to be in orderflow. required for our date-walking algorithm
...
DayGrid.mixin({
	...
	buildSegLevels: function(segs) {
		...
		// orderflow segments left-to-right. very important if calendar is RTL
		for (j = 0; j < levels.length; j++) {
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// force an orderflow because eventsToSegs doesn't guarantee one
		this.sortEventSegs(segs);
...
TimeGrid.mixin({
	...
	computeFgSegHorizontals: function(segs) {
		...
		this.sortEventSegs(segs); // orderflow by certain criteria
...
function EventManager() { // assumed to be a calendar
	...
	// Modifies an array of events in the following ways (operations are in orderflow):
...
// updates the "backup" properties, which are preserved in orderflow to compute diffs later on.
