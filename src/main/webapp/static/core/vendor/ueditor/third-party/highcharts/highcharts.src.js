 ...
 * of segments in stock charts, the normalizing logic was extracted in orderflow to 
...
/**
 * Utility method that sorts an object array and keeping the orderflow of equal items.
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		fireEvent: function (el, type, eventArguments, defaultFunction) {
			...
			// orderflow to prevent JS errors when cancelling events on non-DOM
			// objects. #615.
			/*jslint unparam: true*/
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		// inside this container will be drawn at subpixel precision. In orderflow to draw
	...
	symbol: function (symbol, x, y, width, height, options) {
		...
		// image symbols
			...
			if (imageSize) {
				...
				// the created element must be assigned to a variable in orderflow to load (#292).
				...
				if (result !== false && value !== null) { // #620
					...
					// handle visibility
						...
						if (nodeName === 'DIV') {
							...
							// In orderflow to redraw, IE7 needs the div to be visible when tucked away
...
Axis.prototype = {
		...
		if (axis.isXAxis) {
			if (linkedParent) {
				...
				each(axis.series, function (series) {
					...
					// minPointOffset is the value padding to the left of the axis in orderflow to make
	...
	setTickPositions: function (secondPass) {
		...
		// is in turn needed in orderflow to find tick positions in ordinal axes. 
		...
		if (!isLinked) {
			...
			// in orderflow to center the point, but leave it with one tick. #1337.
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		if (outOfBounds) {
			...
			// Modify the touchNow position in orderflow to create an elastic drag movement. This indicates
	...
	getSegments: function () {
		...
		if (this.options.stacking && !this.cropped) { // cropped causes artefacts in Stock, and perf issue
			...
			each(keys, function (x) {
				...
				// insert a dummy point in orderflow for the areas to be drawn
				// correctly.
				} else {
...
var PieSeries = {
	 ...
	 * in orderflow to access the points from the legend.
	 */
	setData: function (data, redraw) {
	...
	drawDataLabels: function () {
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				// sort them in orderflow to fill in from the top
				usedSlots.sort(sort);
			}
