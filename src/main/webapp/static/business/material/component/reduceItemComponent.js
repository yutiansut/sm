//减项组件js
var reduceItemComponent = Vue.extend({
    template: '#reduceItemTmpl',
    data: function () {
        return {
            //一级分类集合
            topCatalogList: [],
    ...
    methods: {
        ...
        dealCatalogList: function (firstLoadCatalog, catalogList) {
            ...
            //计算sku用量
            materialIndex.dealSkuAmount(self.lowerCatalogList, 3, true);
        },
        //添加商品model--主材
        ...
        removeSku: function (projectMaterialList, projectMaterial, isQuota) {
            var self = this;
            //调用主组件移除商品sku方法
            materialIndex.removeSku(projectMaterialList, projectMaterial, 3, isQuota);
        },
        //修改备注
    ...
    watch: {
        'baseInstallQuotaList': function (newVal,oldVal) {
            var self = this;
        ...
        'titleSum': function (newVal,oldVal) {
            var self = this;
            //更改主组件的titleSum值的显示
            materialIndex.replaceTitleName(3, self.titleSum);
        },
        //监听二级分类的变化:如果是变更:
